...
namespace PolyGone
{
    internal class Entity : Sprite
    {
        ...
        public Entity(Texture2D texture, Vector2 position, int[] size, Color color, Rectangle? srcRect, Dictionary<Vector2, int> collisionMap)
        ...
        public virtual void HandleVerticalCollision(ref bool isOnGround, ref float changeY, List<(Rectangle, CollisionType)> collisions)
        {
            ...
            switch (colType)
            {
                ...
                case CollisionType.SemiSolid:
                    if (changeY > 0 && (position.Y + size[1]) <= tileRect.Top + 10)
                    {
                        // Falling down - land on top of tile
                        position.Y = tileRect.Top - size[1];
                        changeY = 0;
        ...
        public virtual void HandleHorizontalCollision(ref float changeX, List<(Rectangle, CollisionType)> collisions)
        {
            var (tileRect, colType) = collisions[0];
            switch (colType)
            {
        ...
        // Handle entity physics and collisions
        public virtual void Physics(float deltaTime)
        {
            // Apply gravity
            changeY += 0.7f;
